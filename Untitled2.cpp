...
window(80,2,80,23);
for(i=0;i<21;i++)
printf("?"); /* THE BOUNDARY IS DRAWN */
window(1,1,80,25);
goto xy(38;12);
printf("?????"); /* THE "SNAKE" IS PUT for THE FIRST TIME */
goto xy(1;24);
...
goto xy(30;24);
delline();
delline(); /* REMOVING MESSAGE */
printf("\n( EAT THE NUMBER !! ) Score = 0");
goto xy(43;12); /* GO TO THE HEAD OF THE SNAKE */
}
...
printf(fp,"%d %c",length,ch);
for(i=0;i<length;i++)
printf(fp,"%c%c",px[i],py[i]);
fclose(fp);
}
...
scanf(fp,"%d %c",&length,&pch);
for(i=0;i<length;i++){
/* fscanf(fp,"%d %d ",&px[length],&py[length]);*/
scanf(fp,"%c%c",&px[i],&py[i]);
goto xy(px[i]-1;py[i]-1);
putchar('?');
}
window(1,1,80,25);
goto xy(30;24);
...
//cscanf: WHEN YOU PRESS ENTER, CURSOR MOVES TO
